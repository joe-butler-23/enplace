...
const fractions: Record<string, [number, number]> = { '¼': [1,4], '½': [1,2], '¾': [3,4], '⅐': [1,7], '⅑': [1,9], '⅒': [1,10], '⅓': [1,3], '⅔': [2,3], '⅕': [1,5], '⅖': [2,5], '⅗': [3,5], '⅘': [4,5], '⅙': [1,6], '⅚': [5,6], '⅛': [1,8], '⅜': [3,8], '⅝': [5,8], '⅞': [7,8] };
export function parseAmount(text: string, exact = false): RecipeAmount | null {
  const value = text.trim();
  if (!value) return null;
  const match = /^(-\s*)?(?:(\d+)\s+(\d+)\s*\/\s*(\d+)|(\d+)\s+([¼-¾⅐-⅞])|(\d+)\s*\/\s*(\d+)|([¼-¾⅐-⅞])|((?:\d*[.,])?\d+))(.*)$/s.exec(value);
  if (!match) throw new Error(`Invalid RecipeMD amount: ${text}`);
  const unicode = fractions[match[6] || match[9]];
  const whole = match[2] || match[5] || "0";
  const numerator = match[3] || match[7] || (unicode ? String(unicode[0]) : null);
  const denominator = match[4] || match[8] || (unicode ? String(unicode[1]) : null);
  let factor: string;
  if (exact) {
    if (numerator && denominator) {
      const d = BigInt(denominator);
      if (!d) throw new Error(`Invalid RecipeMD amount: ${text}`);
      factor = `${BigInt(whole) * d + BigInt(numerator)}/${d}`;
    } else factor = match[10].replace(',', '.');
    if (match[1]) factor = `-${factor}`;
  } else {
    const numeric = numerator && denominator ? Number(whole) + Number(numerator) / Number(denominator) : Number(match[10].replace(',', '.'));
    if (!Number.isFinite(numeric)) throw new Error(`Invalid RecipeMD amount: ${text}`);
    factor = String((match[1] ? -1 : 1) * numeric);
  }
  return { factor, unit: match[11].trim() || null };
}
...
function ingredient(item: Tokens.ListItem, exactAmounts = false): RecipeIngredient {
  ...
  if (inlines?.[0]?.type === 'em') {
    amount = parseAmount((inlines[0] as Tokens.Em).text, exactAmounts);
...
/** Parse one shopping ingredient; exactAmounts retains fractions for arithmetic instead of decimal display. */
export function parseRecipeIngredient(text: string, exactAmounts = false): RecipeIngredient {
  ...
  return ingredient((list as Tokens.List).items[0], exactAmounts);
}
...
export function parseRecipeMD(markdown: string): RecipeMD {
  ...
  while (metadata(tokens[0])) {
    ...
    if (value.type === 'em') {
      ...
      yields = true; recipe.yields = splitComma(value.text).map((text) => parseAmount(text)).filter((amount): amount is RecipeAmount => amount !== null);
    }
  }
  ...
  while (tokens.length && tokens[0].type !== 'hr') {
    ...
    if (token.type === 'heading') {
      ...
      (stack[stack.length - 1]?.group.ingredients ?? recipe.ingredients).push(...(token as Tokens.List).items.map((item) => ingredient(item)));
