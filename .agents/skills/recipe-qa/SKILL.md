...
1. Resolve the requested recipe in the connected cookbook with `mep list --json`. For a batch, sort paths before review.
2. Read it with `mep show <path>`. For an explicitly supplied local file, read it and validate with `mep check <file> --folder <folder>` instead.
