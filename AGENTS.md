...
- The optional `mep` Node CLI lives in `cli/`. Its live cookbook operations use `src/agent/operations.ts` and one encrypted `src/agent/session.ts` connection per MCP conversation. Explicit folder commands still import the same `src/core.ts`; there is no folder mirror. `docs/agent-integration.md` owns the tool, credential and restricted-agent boundaries.
