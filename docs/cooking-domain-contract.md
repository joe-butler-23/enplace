...
Recipe grouping displays original requirements without amount markup. Aisle and None views show quantities first, merging plural, dialect and preparation variants. Count-part wording (cloves, heads, bunches) supplies a unit only when the explicit amount has none; measurement units are never converted. Plain olive oils and culinary salts share purchase rows, but salt varieties retain separate totals. Different resolved aisles and packaged versus loose forms stay separate. Exact quantities sum only within compatible units; unquantified prose and ranges stay verbatim. Expand Recipe requirements for the originals. Ticking or removing a merged row updates all constituents; it is checked only when all are checked. Grouping and Hide done are browser preferences and never rewrite list order.
