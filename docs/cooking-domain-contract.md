...
Live application writes transform the current shared text inside one Yjs transaction. Saving a recipe draft merges it against the current shared text, preserving overlapping changes as explicit conflict blocks. The optional filesystem mirror applies the same inline three-way merge to text. It stores only merge ancestry and folder association under the reserved `.mep-mirror/`: exact per-path baseline content under `baselines/`, plus `association.json` with the cookbook id and relay. This is recovery metadata, not a second content authority. It lets a restart distinguish a stopped local edit or deletion from unknown initial peer content. Before replacing or deleting a regular file, the mirror stages the desired bytes and atomically moves the current inode to a final same-parent recovery name at `.mep-mirror/<private-operation>/<stem>.local-<timestamp><extension>`. The exact `.mep-mirror` component is never imported. Its directories are tightened to `0700`; state files are atomically published with mode `0600`, and the recovery inode and published replacement retain the displaced target's mode. Final `.local-*` recovery names are never removed automatically, so writes through an already-open descriptor remain recoverable. After restart reaches authoritative public convergence, the mirror removes an exact stale `replacement` staging file only when its bytes match the public file; ambiguous staging remains untouched. Review a recovered file and copy it back deliberately when it contains a wanted local version.
