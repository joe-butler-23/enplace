...
`Shopping.md` at the cookbook root is a Markdown checklist. The browser derives it from the distinct recipes scheduled in the planner's displayed week, rebuilding on each schedule change and when a scheduled recipe's ingredients change; changing the displayed week rebuilds immediately, with no timer. Live `mep shop --week` remains an explicit build for its selected week, while `mep shop --folder <folder>` selects recipes by date and their order in each date section. Building writes one `## <recipe title>` section per contributing recipe, preserving each RecipeMD ingredient item verbatim, including its italic amount, except plain water and ice which are omitted from automatic builds. Manually added water and purchased waters remain. Equal ingredient text is deduplicated only within that recipe block. Each generated recipe block has one adjacent HTML comment with its percent-encoded recipe path and its generated, trimmed case-folded row keys. Checked state carries over by that path and row key; rebuilds change only rows whose key appears in that comment, preserving other content byte-for-byte. Existing non-recipe headings and hand-written content remain unchanged.

Recipe grouping displays original requirements without amount markup. Aisle and None views show quantities first, merging plural, dialect and preparation variants. Count-part wording (cloves, heads, bunches) supplies a unit only when the explicit amount has none. Plain olive oils and culinary salts share purchase rows. Salt shows one total: compatible units sum exactly; mixed weight and volume use an approximate shopping estimate ([6 g/tsp](https://www.kingarthurbaking.com/blog/2022/02/18/digital-kitchen-scale)), marked ≈; unmeasured salt adds no invented amount. Incompatible salt units show just “salt”. Original recipe amounts remain the cooking authority. Different resolved aisles and packaged versus loose forms stay separate. Other products sum only within matching units; unquantified prose and ranges stay verbatim. Ticking or removing a merged row updates all constituents; it is checked only when all are checked. Grouping and Hide done are browser preferences and never rewrite list order.
