...
- `src/recipe-import/` turns outside material into new cookbook files and never overwrites: `paste-import.ts` validates RecipeMD and writes it with an optional cover; `page-import.ts` reads a pasted or opened HTML page with the vendored RecipeClipper module (`recipe-clipper.js`, MIT, synced from that repository, no network) and renders each published recipe as RecipeMD for the chooser in `views/components/ImportPageDialog.tsx`.
