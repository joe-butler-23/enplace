...
function roomName(requestUrl) {
  ...
  if (!ROOM_ID_PATTERN.test(name)) throw new Error("invalid kitchen room");
  return name;
}
...
function applyRoomUpdate(room, update, socket, maxDocumentBytes) {
  ...
  try {
    ...
    if (Y.encodeStateAsUpdate(candidate).byteLength > maxDocumentBytes) {
      socket.close(1009, "kitchen document too large");
      return false;
    }
...
function parseArguments(arguments_) {
  ...
  const usage = "Usage: node scripts/kitchen-relay.mjs [--port N] [--host 127.0.0.1] [--persist <dir>] "
